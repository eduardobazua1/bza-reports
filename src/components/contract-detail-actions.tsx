            ...
            <tbody className="divide-y divide-stone-50">
              {contract.purchaseOrders.map(({ po, invoiceCount, shippedTons }) => (
                <tr key={po.id} className="hover:bg-stone-50">
                  ...
                  <td className="px-4 py-2.5 text-xs text-[#0d3d3b]">{formatDate(po.poDate)}</td>
                  <td className="px-4 py-2.5 text-xs text-right text-stone-600">{formatNumber(po.plannedTons, 1)}</td>
                  <td className="px-4 py-2.5 text-xs text-right text-[#0d3d3b]">{formatNumber(shippedTons, 1)}</td>
