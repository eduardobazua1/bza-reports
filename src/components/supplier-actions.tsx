...
function F({ label, name, dv, ph, type }: { label: string; name: string; dv?: string | null; ph?: string; type?: string }) {
  return (
    <div>
      <label className="block text-xs text-[#0d3d3b] mb-1">{label}</label>
    ...
    <div className="bg-white rounded-md shadow-sm border-l-[3px] border-l-[#0d3d3b] p-5">
      ...
      <form onSubmit={e => { e.preventDefault(); onSave(new FormData(e.currentTarget)); }} className="space-y-6">
        ...
        <div className="flex gap-2">
          <button type="submit" disabled={isPending} className="bg-primary text-primary-foreground px-4 py-2 rounded-lg text-sm font-medium hover:opacity-90 disabled:opacity-50 transition-opacity">
            ...
            <tbody>
              ...
              {suppliers.map(s => (
                <tr key={s.id} className="hover:bg-muted/50 transition-colors">
                  ...
                  <td className="p-3 text-xs text-[#0d3d3b] border-t border-border">{s.contactName || "-"}</td>
                  <td className="p-3 text-xs text-[#0d3d3b] border-t border-border">{s.contactEmail || "-"}</td>
                  <td className="p-3 text-xs text-[#0d3d3b] border-t border-border">{[s.city, s.country].filter(Boolean).join(", ") || "-"}</td>
      ...
      {openDropdownId !== null && dropdownPos && (() => {
        ...
        return createPortal(
          <div ref={dropdownRef} style={{ position: "fixed", top: dropdownPos.top, right: dropdownPos.right, zIndex: 9999 }} className="bg-white border border-stone-200 rounded-md shadow-lg min-w-[130px] py-1 text-left">
            ...
            <button onClick={() => { setOpenDropdownId(null); handleDelete(supplier.id); }} disabled={isPending} className="flex items-center gap-2 w-full text-left px-4 py-2 text-sm text-stone-600 hover:bg-stone-50"><Trash2 className="w-3.5 h-3.5" />Delete</button>
          </div>,
          document.body
        );
      })()}
