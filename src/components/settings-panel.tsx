        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-stone-100">
          ...
          {["company","invoice"].includes(active) && (
            <div className="flex items-center gap-3">
              {saved && (
                <span className="text-xs text-[#0d3d3b] font-medium flex items-center gap-1">
        ...
        <div className="flex items-start gap-8 flex-wrap">
          ...
          <div className="flex items-end">
            <button onClick={() => { set("primaryColor", "#0d3d3b"); set("accentColor", "#4fd1c5"); }}
              className="text-xs text-[#0d3d3b] hover:text-[#0d3d3b] border border-stone-200 rounded-lg px-3 py-2 transition-colors">
              ↺ BZA defaults
            </button>
          </div>
        </div>
  ...
  return (
    <>
      <Card title="Team Members">
          ...
          {users.map(u => (
            <div key={u.id} className="flex items-center justify-between py-2 px-3 bg-white rounded-lg border border-stone-100 gap-2 flex-wrap">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className={`text-[11px] px-2 py-0.5 rounded-full ${u.isActive ? "bg-[#e6f1ee] text-[#0d3d3b]" : "bg-stone-200 text-stone-500"}`}>{u.isActive ? "Active" : "Inactive"}</span>
                {isAdmin && (
                  <>
                    <button
                      disabled={busy === u.id}
        ...
        {err && <p className="text-xs text-stone-600 mt-2">{err}</p>}
