...
type VisibleCols = {
  date: boolean; type: boolean; invoice: boolean;
  amount: boolean; openBalance: boolean; days: boolean;
};
type ViewMode = "grouped" | "flat";
type ToastState = { message: string; type: "success" | "error" } | null;

const STORAGE_KEY = "bza_cols_ap-aging-detail";

const COL_DEFS: { key: keyof VisibleCols; label: string }[] = [
  { key: "date",        label: "Date" },
  { key: "type",        label: "Type" },
...
function EmailModal({ onClose, onSend, isSending }: { onClose: () => void; onSend: (to: string, subject: string, message: string) => void; isSending: boolean }) {
  const [to, setTo] = useState(""); const [subject, setSubject] = useState("A/P Aging Detail — BZA International Services"); const [message, setMessage] = useState("");
  return (
...
export function APAgingDetailFullClient({ buckets, total, filterBucket, filterSupplier, asOf, timestamp }: {
  buckets: Bucket[]; total: number; filterBucket?: string; filterSupplier?: string; asOf: string; timestamp: string;
}) {
  ...
  const [visibleCols, setVisibleCols] = useState<VisibleCols>(() => {
    ...
    return { date: true, type: true, invoice: true, amount: true, openBalance: true, days: true };
  });
  ...
  const [sortCol, setSortCol] = useState<"date" | "invoice" | "supplier" | "amount" | "days" | null>(null);
  const [sortDir, setSortDir] = useState<"asc" | "desc">("asc");

  function handleSort(col: "date" | "invoice" | "supplier" | "amount" | "days") {
  ...
  function sortRows(rows: Row[]) {
    ...
    return [...rows].sort((a, b) => {
      ...
      switch (sortCol) {
        ...
        case "days":     av = a.daysSince;  bv = b.daysSince;  break;
        default: return 0;
      }
  ...
  const allRows: Row[] = active.flatMap(b => filterSupplier ? b.rows.filter(r => r.supplier.toLowerCase().includes(filterSupplier.toLowerCase())) : b.rows);
  ...
  function buildExportHeaders() {
    ...
    if (visibleCols.days) h.push("Days");
    return h;
  }
  ...
  function buildExportRows() {
    return allRows.map(r => {
      ...
      if (visibleCols.days) obj["Days"] = r.daysSince > 0 ? r.daysSince : 0;
      return obj;
    });
  }
  ...
  function buildPdfPayload() {
    ...
    return {
      title: "A/P Aging Detail", subtitle: "BZA International Services", dateLabel: `As of ${asOf}`,
      columns: headers.map((h, i) => ({ key: String(i), label: h, align: (["Date", "Type", "Invoice", "Supplier"].includes(h) ? "left" : "right") as "left" | "right", ...(["Amount", "Open Balance"].includes(h) ? { format: "currency" as const } : {}) })),
  ...
  function handlePdf() {
    ...
    startPdf(async () => {
      ...
      const a = document.createElement("a"); a.href = url; a.download = "AP-Aging-Detail.pdf";
  ...
  function handleExcel() {
    ...
    const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "AP Aging Detail");
    XLSX.writeFile(wb, "AP-Aging-Detail.xlsx");
  }
  ...
  function handleCsv() {
    ...
    const a = document.createElement("a"); a.href = url; a.download = "AP-Aging-Detail.csv";
    document.body.appendChild(a); a.click(); document.body.removeChild(a);
    setTimeout(() => URL.revokeObjectURL(url), 10000);
  }
      ...
      {showCustomize && (
        <div className="fixed right-10 top-1/2 -translate-y-1/2 z-[59] bg-white rounded-xl shadow-xl border border-stone-200 p-4 w-56 print:hidden">
          ...
          <div className="space-y-2 mb-4">
            ...
            <p className="text-[10px] text-stone-400 pt-1">Supplier always visible</p>
          </div>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-stone-200 text-xs font-semibold text-stone-500 uppercase tracking-wide select-none">
                  ...
                  <th onClick={() => handleSort("supplier")} className="text-left px-4 py-3 cursor-pointer hover:text-stone-700">Supplier<SI col="supplier" /></th>
                  ...
                  {visibleCols.days        && <th onClick={() => handleSort("days")} className="text-right px-6 py-3 w-24 cursor-pointer hover:text-stone-700">Days<SI col="days" /></th>}
                </tr>
              </thead>
              <tbody>
                {viewMode === "flat" ? (
                  <>
                    {(sortCol ? sortRows(allRows) : [...allRows].sort((a, b) => b.daysSince - a.daysSince)).map((row, i) => (
                      <tr key={i} className="border-t border-stone-50 hover:bg-stone-50">
                        ...
                        {visibleCols.invoice  && <td className="px-4 py-2.5 text-xs text-[#0d3d3b]">{row.num}</td>}
                  ...
                  active.map(bucket => {
                    const bucketRows = sortRows(filterSupplier ? bucket.rows.filter(r => r.supplier.toLowerCase().includes(filterSupplier.toLowerCase())) : bucket.rows);
