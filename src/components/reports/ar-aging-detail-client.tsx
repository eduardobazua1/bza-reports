...
export function ARAgingDetailClient({ buckets, total, filterBucket, filterClient }: {
  buckets: Bucket[];
  total: number;
  filterBucket?: string;
  filterClient?: string;
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-stone-200 text-xs font-semibold text-stone-500 uppercase tracking-wide">
            ...
            <th className="text-right px-6 py-3 w-24">Past Due</th>
          </tr>
        </thead>
        <tbody>
          {active.map(bucket => {
            const bucketRows = filterClient
              ? bucket.rows.filter(r => r.customer.toLowerCase().includes(filterClient.toLowerCase()))
              : bucket.rows;
            ...
            return [
              ...
              ...(isOpen ? [
                <tr key={`sub-${bucket.key}`} className="border-t border-stone-200 bg-stone-50">
                  <td colSpan={5} className="px-6 py-2 text-xs font-semibold text-stone-500">
        ...
        <tfoot>
          <tr className="border-t-2 border-stone-400">
            <td colSpan={5} className="px-6 py-3 text-sm font-bold text-stone-900 uppercase tracking-wide">TOTAL</td>
