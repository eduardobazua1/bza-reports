...
export async function generateInvoicePdf(invoiceNumber: string): Promise<Buffer> {
  ...
  const refCols: { label: string; value: string; x: number; w: number }[] = [
    { label: "PURCHASE ORDER",    value: inv.salesDocument || po.clientPoNumber || po.poNumber, x: M,       w: 110 },
    { label: "VEHICLE / RAILCAR", value: inv.vehicleId || "\u2014",         x: M + 120, w: 100 },
    { label: "BOL #",             value: inv.blNumber || "\u2014",          x: M + 228, w: 80  },
    { label: "DESTINATION",       value: inv.destination || "\u2014",       x: M + 316, w: 130 },
    { label: "SHIP DATE",         value: formatDate(inv.shipmentDate),      x: M + 454, w: 62  },
  ];
  refCols.forEach(({ label, x }) => dt(label, x, y, 6, fontB, GRAY));
  refCols.forEach(({ value, x, w }) => {
    wrapText(value, w, font, 8).forEach((l, i) => dt(l, x, y + 9 + i * 10, 8, font, DARK));
  });
  y += 26;
  ...
  wrapText(productLine, 228, font, 7.5).forEach((l, i) => dt(l, TC.product, y + 9 + i * 10, 7.5, font, DARK));
