...
async function migrate(url: string, authToken?: string) {
  ...
  await client.execute(`
    ALTER TABLE supplier_invoices ADD COLUMN linked_invoice_id INTEGER REFERENCES invoices(id)
  `);

  console.log("✓ linked_invoice_id column added to supplier_invoices");
  await client.close();
}
