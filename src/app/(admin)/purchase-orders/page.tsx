              ...
              {poRows.map((row) => {
                ...
                return (
                  <tr key={row.po.id} className="hover:bg-muted/50 transition-colors">
                    ...
                    <td className="p-3 text-xs text-stone-500 border-t border-border">{formatDate(row.po.poDate)}</td>
                    <td className="p-3 text-xs font-medium text-stone-700 border-t border-border">{row.clientName || "-"}</td>
                    <td className="p-3 text-xs text-stone-500 border-t border-border">{row.supplierName || "-"}</td>
                    <td className="p-3 text-xs font-medium text-stone-700 border-t border-border">{row.productName}</td>
                    <td className="p-3 text-xs font-medium text-stone-700 border-t border-border text-right">{formatCurrency(row.po.sellPrice)}</td>
                    <td className="p-3 text-xs text-stone-500 border-t border-border text-right">{formatCurrency(row.po.buyPrice)}</td>
                    <td className="p-3 text-xs text-stone-400 border-t border-border text-right">{row.invoiceCount}</td>
                    <td className="p-3 text-xs font-medium text-stone-700 border-t border-border text-right">{formatNumber(row.totalTons, 1)}</td>
                    <td className="p-3 text-xs font-medium text-stone-700 border-t border-border text-right">{formatCurrency(revenue)}</td>
