          ...
          <table className="w-full text-xs">
              ...
              {sorted.map((p) => {
                ...
                return (
                  <tr key={p.product} className="border-b border-stone-50 hover:bg-stone-50">
                    <td className="px-6 py-3 text-xs text-[#0d3d3b]">
                      <div>{p.product}</div>
                    ...
                    <td className="px-4 py-3 text-xs text-right text-stone-600">
                      {p.totalTons.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </td>
                    <td className="px-4 py-3 text-xs text-right font-semibold text-[#0d3d3b]">{formatCurrency(p.totalRevenue)}</td>
                    <td className="px-4 py-3 text-xs text-right font-semibold text-[#0d3d3b]">{formatCurrency(p.totalCost)}</td>
                    <td className="px-4 py-3 text-xs text-right font-semibold text-[#0d3d3b]">{formatCurrency(grossProfit)}</td>
            ...
            <tfoot>
              <tr className="border-t-2 border-stone-300 bg-stone-50">
                ...
                <td className="px-4 py-3 text-right font-semibold text-[#0d3d3b]">{formatCurrency(totals.totalRevenue)}</td>
                <td className="px-4 py-3 text-right font-semibold text-[#0d3d3b]">{formatCurrency(totals.totalCost)}</td>
                <td className="px-4 py-3 text-right font-semibold text-[#0d3d3b]">{formatCurrency(totals.grossProfit)}</td>
