...
async function generateInvoicePdf(invoiceNumber: string): Promise<Buffer> {
  ...
  if (cfg.showFscSection !== false) {
    ...
    if (isPefc) {
      doc.fontSize(7).font("Helvetica").fillColor(DARK)
        .text(`PEFC Number: ${po.pefc || "—"}`, M, y, { width: W, lineBreak: false });
    } else {
      doc.fontSize(7).font("Helvetica").fillColor(DARK)
