...
- FSC Claims: ${fscInfo || "-"}`;
    ...
    if (name === "create_invoice") {
      ...
      await db.insert(invoices).values({ invoiceNumber: args.invoiceNumber as string, purchaseOrderId: po.id, quantityTons: args.quantityTons as number, item: (args.item as string) || null, shipmentDate: (args.shipmentDate as string) || null, vehicleId: (args.vehicleId as string) || null, blNumber: (args.blNumber as string) || null, currentLocation: (args.currentLocation as string) || null, destination: (args.destination as string) || null, balesCount: (args.balesCount as number) || null, unitsPerBale: (args.unitsPerBale as number) || null, invoiceDate: (args.invoiceDate as string) || null, salesDocument: (args.salesDocument as string) || null, billingDocument: (args.billingDocument as string) || null, buyPriceOverride: (args.buyPriceOverride as number) ?? null, sellPriceOverride: (args.sellPriceOverride as number) ?? null });
      const tons = args.quantityTons as number;
      const sellUnit = (args.sellPriceOverride as number) ?? po.sellPrice;
      const buyUnit = (args.buyPriceOverride as number) ?? po.buyPrice;
      const totalSell = sellUnit ? tons * sellUnit : null;
      const totalBuy = buyUnit ? tons * buyUnit : null;
      return `Invoice ${args.invoiceNumber} created on PO ${args.poNumber}:
- Tons: ${tons} TN
- Total Revenue: ${totalSell ? `$${totalSell.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : "-"} (${tons} × $${sellUnit}/TN${args.sellPriceOverride != null ? " override" : ""})
- Total Cost: ${totalBuy ? `$${totalBuy.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : "-"} (${tons} × $${buyUnit}/TN${args.buyPriceOverride != null ? " override" : ""})
...
### Step 0 — BEFORE you write anything (do this every time):
- **Auto-number invoices:** run_calculation \`SELECT invoice_number FROM invoices WHERE invoice_number LIKE 'IX{PO}-%'\` to find the highest existing IX{PO}-N on that PO, and continue from N+1. NEVER reuse or skip a number, never guess it.
- **Cross-check the 3 sources:** the supplier INVOICE, the DELIVERY NOTE (BOL), and the COA / PACKING LIST must agree on Load No, Delivery Note #, railcar (Vehicle ID), ADMT and gross kg. If any disagree, STOP and flag it — do not capture.
- **Price override (important):** compare the supplier invoice's Unit price to the PO's buy_price (run_calculation on purchase_orders). If they differ, set **buyPriceOverride = the actual invoiced unit price** so COGS is exact (e.g. PO says 650 but the invoice says 655 → buyPriceOverride 655). Likewise, if a client billing doc shows a net price different from the PO sell_price, set sellPriceOverride.
- **Bundles vs separate files:** documents may come as separate PDFs (one invoice, one delivery note, one packing list) OR as ONE combined bundle per railcar (page 1 invoice, page 2 delivery note, pages 3-5 packing list, last page COA). create_supplier_invoice already takes page 1 only; for a bundle, attach the same bundle file as both bl and pl if you cannot split it, and say so.
...
- When the user uploads an image or PDF, you can SEE it directly (vision) — read all text, tables, and numbers from the page images.
